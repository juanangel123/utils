...
for (i = 3; i <= process.argv.length - 1; i++) {
    if (!process.argv[i]) {
        ...
        devArguments.push(process.argv[i].toString().replace('--', ''));
    }
}
...
if (!themeName) {
    for (var i in themesConfig) {
        // Create path
        pubPath = './pub/static/' + themesConfig[i].area + '/' + themesConfig[i].name + '/' + themesConfig[i].locale + '/';
        appPath = './app/design/' + themesConfig[i].area + '/' + themesConfig[i].name + '/';

        // Push names of less files to the Array
        for (var j in themesConfig[i].files) {
            lessFiles.push(pubPath + themesConfig[i].files[j] + '.' + themesConfig[i].dsl);
        }
    }
}
...
// Get certain theme, create paths for less files and push them to the Array.
    ...
    // Create path
    pubPath = './pub/static/' + themesConfig[themeName].area + '/' + themesConfig[themeName].name + '/' + themesConfig[themeName].locale + '/';
    appPath = './app/design/' + themesConfig[themeName].area + '/' + themesConfig[themeName].name + '/';

    // Push names of less files to the Array
    for (var i in themesConfig[themeName].files) {
        lessFiles.push(pubPath + themesConfig[themeName].files[i] + '.' + themesConfig[themeName].dsl)
    }
}
...
gulp.task('watch', function () {
    ...
    gulp.watch([appPath + '**/*.less'], gulp.parallel('less'));
});
